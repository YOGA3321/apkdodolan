...
$query = mysqli_query($Koneksi, "SELECT tb_order.*,tb_bayar.*,nama, SUM(harga*jumlah) AS harganya, tb_list_order.status AS status FROM tb_order 
                        ...
                        <?php
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a class="btn btn-info btn-sm me-1" href="./?x=orderitem&order=<?php echo $row['id_odr']; ?>&meja=<?php echo $row['meja']; ?>&pelanggan=<?php echo $row['pelanggan']; ?>"><img src="../image/eye.svg"></a>
                                            <button class="btn <?php echo (!empty($row['id_bayar']) || $row['status_list_order'] == 1 || (strtotime($row['waktu_order']) + 7 * 60) <= time()) ? 'btn-secondary btn-sm me-1 disabled' : 'btn-warning btn-sm me-1'; ?>" data-bs-toggle="modal" data-bs-target="#ModalEdit<?php echo $row['id_odr'] ?>"
                                                <?php echo (!empty($row['id_bayar']) || $row['status_list_order'] == 1 || (strtotime($row['waktu_order']) + 10 * 60) <= time()) ? 'disabled' : ''; ?>>
                                                <img src="../image/pencil-square.svg">
                                            </button>
                                            <button class="btn <?php echo (!empty($row['id_bayar']) || $row['status_list_order'] == 1 || (strtotime($row['waktu_order']) + 7 * 60) <= time()) ? 'btn-secondary btn-sm me-1 disabled' : 'btn-danger btn-sm me-1'; ?>" data-bs-toggle="modal" data-bs-target="#ModalDelete<?php echo $row['id_odr'] ?>"
                                                <?php echo (!empty($row['id_bayar']) || $row['status_list_order'] == 1 || (strtotime($row['waktu_order']) + 7 * 60) <= time()) ? 'disabled' : ''; ?>>
                                            ...
                                            <div class="modal-body">
                                                <form action="" method="POST">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <button type="submit" class="btn btn-danger" name="hapus_order">Hapus</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
